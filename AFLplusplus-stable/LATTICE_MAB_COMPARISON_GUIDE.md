# Lattice-MAB 对比测试完整指南

本指南详细介绍如何运行 Lattice-MAB 并与 AFL++ 原始策略进行对比测试。
...
```bash
# 检查 afl-fuzz 是否包含 Lattice-MAB
./afl-fuzz -h | grep -i lattice || echo "Lattice-MAB 已集成"
```
...
# 使用 afl-cc 编译
./afl-cc -o test_target test_target.c
...
**完整命令路径：**
```bash
"/Users/ketangchen/Library/Mobile Documents/com~apple~CloudDocs/Documents/000_20250825dev/laboratory_library/AFLplusplus-stable/afl-fuzz" -i seeds -o output_with_lm -- ./test-instr @@
```
...
**完整命令路径：**
```bash
AFL_DISABLE_LATTICE_MAB=1 "/Users/ketangchen/Library/Mobile Documents/com~apple~CloudDocs/Documents/000_20250825dev/laboratory_library/AFLplusplus-stable/afl-fuzz" -i seeds -o output_without_lm -- ./test-instr @@
```
...
echo ""
echo "详细统计文件位置:"
echo "  启用 Lattice-MAB: $with_stats"
echo "  禁用 Lattice-MAB: $without_stats"
EOF
...
#   -t: 目标二进制文件路径
#   -i: 输入种子目录
#   -T: 每个测试运行时间（秒），默认300秒（5分钟）
#   -n: 每个配置运行次数，默认3次
#   -o: 输出目录，默认 ./comparison_results
...
自动化脚本会：
1. 运行启用 Lattice-MAB 的测试 N 次
2. 运行禁用 Lattice-MAB 的测试 N 次
3. 计算平均值和提升百分比
...
### 示例 1: 简单测试程序（test-instr.c）

**文件路径**: `/Users/ketangchen/Library/Mobile Documents/com~apple~CloudDocs/Documents/000_20250825dev/laboratory_library/AFLplusplus-stable/test-instr.c`
...
# 准备种子
mkdir -p seeds
echo "<test>hello</test>" > seeds/seed1.xml

# 运行对比测试
../afl-fuzz -i seeds -o output_with_lm -- ./xmllint @@
AFL_DISABLE_LATTICE_MAB=1 ../afl-fuzz -i seeds -o output_without_lm -- ./xmllint @@
```
